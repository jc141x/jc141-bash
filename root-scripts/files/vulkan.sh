...
dxvk() { DXVKVER="$(curl -s -m 5 https://api.github.com/repos/jc141x/dxvk/releases/latest | awk -F '["/]' '/"browser_download_url":/ {print $11}' | cut -c 2-)"; SYSDXVK="$(command -v setup_dxvk)"
    dxvk() {
    [ -n "$SYSDXVK" ] && echo "using local dxvk" && "$SYSDXVK" install --symlink > /dev/null && wineserver -w && applied
    [ -z "$SYSDXVK" ] && echo "using external dxvk" && github_dxvk && echo "$DXVKVER" >"$WINEPREFIX/.dxvk"; }
...
[[ -z "$(status)" ]] && dxvk-async
[[ -f "$WINEPREFIX/.dxvk-async" && -n "$DXVKVER" && "$DXVKVER" != "$(awk '{print $1}' "$WINEPREFIX/.dxvk-async")" ]] && { rm -f dxvk-async-*.tar.gz || true; } && echo -n "updating dxvk-async." && dxvk-async
...
vkd3d() { VKD3DVER="$(curl -s -m 5 https://api.github.com/jc141x/vkd3d-proton/releases/latest | awk -F '["/]' '/"browser_download_url":/ {print $11}' | cut -c 2-)"
    ...
    vkd3d() {
        [ -n "$SYSVKD3D" ] && echo "using local vkd3d." && "$SYSVKD3D" install --symlink > /dev/null && wineserver -w && applied
        [ -z "$SYSVKD3D" ] && echo "using external vkd3d." && github_vkd3d && echo "$VKD3DVER" >"$WINEPREFIX/.vkd3d"; }
