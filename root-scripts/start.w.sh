...
cd "$(dirname "$(readlink -f "$0")")" || exit; [ "$EUID" = "0" ] && exit; export R="$PWD"; DWRF="$R/dwarfsettings.sh"; WHA="$R/files/wha.sh"; VLK="$R/files/vulkan.sh"; LOGO="$PWD/files/logo.txt.gz"
...
bash "$DWRF" mount-game; bash "$DWRF" mount-prefix; echo ''
