...
```sh
# wine-ge
export WINE="$JCD/wine/wine-ge/wine/bin/wine" && { ping -c 3 github.com >/dev/null || { echo "Github could not be reached, probably no network or ping command was blocked. The game may or may not fail to run if wine-ge was never downloaded before." ; }; GEVER="$(curl -s -m 5 https://api.github.com/repos/jc141x/wine-ge-custom/releases/latest | awk -F '["/]' '/"browser_download_url":/ {print $11}' | cut -c 1-)"
