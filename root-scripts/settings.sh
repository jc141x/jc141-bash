...
extract-dwarfs() { [ -d "$PWD/files/groot" ] && [ "$( ls -A "$PWD/files/groot")" ] && echo "Game is already mounted or extracted." && exit; mkdir "$PWD/files/groot"; echo "Started extraction process. Only done at first run. The game will not run mounted due to the unlikelyness of running smoothly based on our judgment. If the process is closed before the extraction finishes, then the script will try to run the incomplete files next time. Have patience or delete files/groot directory before trying again." && dwarfsextract -i "$PWD/files/groot.dwarfs" -o "$PWD/files/groot"; }
