...
extract-dwarfs() { [ -d "$PWD/files/groot" ] && [ "$( ls -A "$PWD/files/groot")" ] && echo "Game is already mounted or extracted." && exit; mkdir "$PWD/files/groot"; echo "Started extraction process. If the process is closed before the extraction finishes, then the script will try to run the incomplete files. Have patience or delete files/groot directory and run again if the process was canceled." && dwarfsextract -i "$PWD/files/groot.dwarfs" -o "$PWD/files/groot"; }
