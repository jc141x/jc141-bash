...
extract-dwarfs() { fusermount3 -u "$PWD/files/groot-mnt"

[ -d "$PWD/files/groot" ] && [ "$( ls -A "$PWD/files/groot")" ] && echo "game is already mounted or extracted" && exit || echo "extracting game files, this can take a while" && mkdir "$PWD/files/groot" & mkdir "$PWD/files/groot-mnt"; dwarfs "$PWD/files/groot.dwarfs" "$PWD/files/groot-mnt" -o clone_fd && cp -r "$PWD/files/groot-mnt"/* "$PWD/files/groot" && fusermount3 -u "$PWD/files/groot-mnt" && rm -Rf "$PWD/files/groot-mnt"; }
