...
mount-game() {
[ -d "$PWD/files/groot" ] && echo "DWRFS: Mounting path exists." && [ ! ls -A "$PWD/files/groot" ] && echo "DWRFS: Game is already mounted or extracted." || mkdir -p {"$PWD/files/groot-mnt","$PWD/files/groot-rw","$PWD/files/groot-work","$PWD/files/groot"} && dwarfs "$PWD/files/groot.dwarfs" "$PWD/files/groot-mnt" && fuse-overlayfs -o lowerdir="$PWD/files/groot-mnt",upperdir="$PWD/files/groot-rw",workdir="$PWD/files/groot-work" "$PWD/files/groot" && echo "DWRFS: Mounted game."; }
...
extract-game() { [ -d "$PWD/files/groot" ] && echo "DWRFS: Extraction path exists." && [ ! ls -A "$PWD/files/groot" ] && echo "DWRFS: Game is already extracted." && exit || rm -d "$PWD/files/groot" && echo "DWRFS: Extracting game, this may take a while. Do not cancel the process or the files will be incomplete."; tstart="$(date +%s)" && mkdir "$PWD/files/groot" && dwarfsextract -i "$PWD/files/groot.dwarfs" -o "$PWD/files/groot" && tend="$(date +%s)"; elapsed="$((tend - tstart))" && echo "done in $((elapsed / 60)) min and $((elapsed % 60)) sec"; }
