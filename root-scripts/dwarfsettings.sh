...
[ -d "$PWD/files/groot" ] && echo -n "DWRFS: Mounting path exists. | " && [ "$( ls -A "$PWD/files/groot")" ] && echo -n "DWRFS: Game is already mounted or extracted. | " && exit
mkdir -p {"$PWD/files/groot-mnt","$PWD/files/groot-rw","$PWD/files/groot-work","$PWD/files/groot"} && dwarfs "$PWD/files/groot.dwarfs" "$PWD/files/groot-mnt" && fuse-overlayfs -o lowerdir="$PWD/files/groot-mnt",upperdir="$PWD/files/groot-rw",workdir="$PWD/files/groot-work" "$PWD/files/groot" && echo -n "DWRFS: Mounted game. | "; }
...
[ ! -f "$RMTDIR/$RMTARCH" ] && [ ! -d "$RMTCONTENT" ] && curl -L "$DLRLS" -o "$RMTDIR/$RMTARCH" && [ ! -f "$RMTDIR/$RMTARCH" ] && echo -n "RMT: Download failed. | " && exit || [ ! -d "$RMTCONTENT" ] && tar -xvf "$RMTDIR/$RMTARCH" -C "$RMTDIR" > /dev/null && rm -Rf "$RMTDIR/$RMTARCH"
