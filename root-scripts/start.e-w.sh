...
# bwrap
[ ! -f "/usr/bin/bwrap" ] && echo "BWRAP package not installed, no filesystem isolation." && exit || [ "${BWRAP:=1}" = "0" ] && echo "BWRAP isolation is not enabled due to user input." || { export BWRP="bwrap --bind / / --dev-bind /dev /dev --bind "$WINEPREFIX/drive_c/users/$USER" "$JCD/universal-saves-and-configuration" --bind "$PWD/files/groot-rw" "$JCD/universal-saves-and-configuration""; echo "BWRAP isolation enabled." && echo "Path of universally isolated (BWRAP) saves and configurations is "$JCD/universal-saves-and-configuration""; }
